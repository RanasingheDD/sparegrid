...
def send_welcome_email(*, user_email: str, user_name: str) -> None:
    subject, body = email_templates.welcome_email(user_name)
...
def send_new_order_notifications(*, order_id: str, buyer_name: str, buyer_email: str, product_title: str, quantity: int, shipping_address: str, item_cost: float) -> None:
    admin_subject, admin_body = email_templates.new_order_admin_email(
        order_id=order_id,
    ...
    buyer_subject, buyer_body = email_templates.new_order_buyer_email(
        name=buyer_name,
        order_id=order_id,
        product_title=product_title,
        quantity=quantity,
    ...
    send_email(settings.SUPPORT_EMAIL, admin_subject, admin_body)
...
def send_new_product_notifications(*, seller_email: str, seller_name: str, product_id: str, title: str, price: float, category: str) -> None:
    admin_subject, admin_body = email_templates.new_product_admin_email(
    ...
    seller_subject, seller_body = email_templates.new_product_seller_email(
        name=seller_name,
        title=title,
        price=price,
        category=category,
...
def send_product_review_email(*, seller_email: str, seller_name: str, title: str, status: ProductStatus, item_cost: float) -> None:
    subject, body = email_templates.product_review_email(seller_name, title, status, item_cost)
...
def send_order_status_email(*, buyer_email: str, buyer_name: str, order_id: str, product_title: str, item_cost: float, status: DeliveryStatus, tracking_notes: str | None = None) -> None:
    subject, body = email_templates.order_status_email(
        name=buyer_name,
        order_id=order_id,
        product_title=product_title,
        item_cost=item_cost,
